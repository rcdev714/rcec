...
// GET /api/feed - Get personalized feed (public posts + followed users' posts)
export async function GET(request: Request) {
  try {
    ...
    // 2. Followers-only posts from users we follow
    // 3. All posts from current user
    const query = followedUserIds.length > 0
      ? `visibility.eq.public,and(visibility.eq.followers,author_id.in.(${followedUserIds.join(',')})),author_id.eq.${user.id}`
      : `visibility.eq.public,author_id.eq.${user.id}`;
